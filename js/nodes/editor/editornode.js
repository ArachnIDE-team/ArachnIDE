...
class WebEditorNode extends WindowedNode {
    ...
    constructor(configuration = WebEditorNode.DEFAULT_CONFIGURATION){
        ...
        if(configuration.savedV2) {
            ...
            if(configuration.content === undefined) {// Create WebEditorNode
                configuration.content = WebEditorNode._getContentElement();
                configuration.content = [configuration.content];
                super({title: configuration.name, content: configuration.content, ...WindowedNode.getNaturalScaleParameters()});
                htmlnodes_parent.appendChild(this.content);
                registernode(this);
                this._initialize(configuration.name, false)
            } else {// Restore WebEditorNode
                super({title: configuration.name, content: configuration.content, scale: true, saved: true, saveData: configuration.saveData})
            }
        }
    ...
    _initialize(name, saved){
        ...
        if(!saved){
            this.isEditorNode = true;

            this.editorSaveData = null;
        }
    ...
    afterInit() {
        ...
        iframeElement.onload = () => {
            ...
            if (iframeWindow.htmlEditor && iframeWindow.cssEditor && iframeWindow.jsEditor) {
                ...
                const debounceSave = debounce(() => this.saveEditorContent(), 300);
    ...
    saveEditorContent() {
        ...
        if (iframeElement && iframeElement.contentWindow) {
            ...
            //console.log('Editor content saved:', this.editorSaveData);
        }
    }

    restoreEditorContent() {
        if (!this.editorSaveData) {
        ...
        if (iframeElement && iframeElement.contentWindow) {
            try {
                ...
                let iframeWindow = iframeElement.contentWindow;
                iframeWindow.htmlEditor.setValue(this.editorSaveData.html || '');
                iframeWindow.cssEditor.setValue(this.editorSaveData.css || '');
                iframeWindow.jsEditor.setValue(this.editorSaveData.js || '');
            } catch (error) {
...
function createEditorNode(title = '', sx = undefined, sy = undefined, x = undefined, y = undefined) {
    ...
    // return new WebEditorNode(title, undefined, sx, sy, x, y);
}
// DONE WebEditorNode._getContentElement
// function createEditorNode(title = '', sx = undefined, sy = undefined, x = undefined, y = undefined) {
//     // Create the wrapper div
...
// DONE WebEditorNode.saveEditorContent
...
// DONE WebEditorNode.restoreEditorContent
