...
function skipAutopilot() {
    background.zoom = zoomTo
    background.pan = autopilotReferenceFrame ? autopilotReferenceFrame.pos.plus(panTo) : panTo;
}
...
function nodeStep(time) {
    ...
    if (autopilotReferenceFrame && autopilotSpeed !== 0) {
        ...
        newPan = background.pan.scale(1 - autopilotSpeed).plus(autopilotReferenceFrame.pos.scale(autopilotSpeed).plus(panToI));
        panToI_prev = autopilotReferenceFrame.pos.scale(1);
        ...
        newPan = background.pan.scale(1 - autopilotSpeed).plus(panTo.scale(autopilotSpeed));
    ...
    if (autopilot_travelDist > settings.autopilotMaxSpeed) {
        ...
        background.zoom = background.zoom.scale(1 - speedCoeff * autopilotSpeed);
        //*Math.log(autopilot_travelDist/settings.autopilotMaxSpeed));
    } else {
        background.zoom = background.zoom.scale(1 - autopilotSpeed).plus(zoomTo.scale(autopilotSpeed));
    }
